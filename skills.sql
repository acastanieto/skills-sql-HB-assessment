...
SELECT m.brand_name, m.name, b.founded FROM Models AS m LEFT JOIN Brands AS b ON (m.brand_name=b.name) WHERE m.year = 1960;
...
-- Left joins include all the rows in the FROM table even if the row doesn't have a value that matches the joined column from the joined table.  The joined table, however, only has rows that match joined column values in the FROM table.  Inner joins only include rows in both tables that have matching values in the joined columns.
...
SELECT brand_name FROM Models WHERE (SELECT COUNT(*) FROM Models) > 5 GROUP BY brand_name;
...
BEGIN;
INSERT INTO Awards (name, year, winner_id)                   VALUES ('IIHS Safety Award', 2015, (SELECT id FROM Models WHERE year = 2015 AND name = 'Malibu'));
INSERT INTO Awards (name, year, winner_id)                   VALUES ('IIHS Safety Award', 2015, (SELECT id FROM Models WHERE year = 2015 AND name = 'Outback'));
COMMIT;

-- 5. Using a subquery, select only the *name* of any model whose
-- year is the same year that *any* brand was founded.
SELECT m.name FROM Models AS m JOIN Brands AS b ON (m.brand_name=b.name)WHERE m.year IN (SELECT founded FROM Brands);
